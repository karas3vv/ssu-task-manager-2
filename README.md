...
```text
src/app/        маршруты Next.js, layouts, API routes, metadata-файлы
src/entities/   бизнес-сущности и общие model-типы
src/features/   пользовательские фичи, логика и компоненты
src/shared/     API-клиент, конфиги, providers, stores, SEO helpers, UI-утилиты
src/widgets/    крупные блоки страниц и составные UI-секции
src/i18n/       конфигурация next-intl и сообщения переводов
```
...
Все клиентские API-запросы выполняются через типизированный axios-клиент в `src/shared/api`.
